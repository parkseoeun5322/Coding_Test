...
// 백준 온라인 저지 1744번 / 교재 203 페이지

// 문제 : 길이가 N인 수열이 주어졌을 때, 그 수열의 합을 구하려고 한다. 
...
// 입력 : 첫째 줄에 수열의 크기 N이 주어진다. N은 50보다 작은 자연수이다. 둘째 줄부터 N개의 줄에 수열의 각 수가 주어진다. 
//		 수열의 수는 -1,000보다 크거나 같고, 1,000보다 작거나 같은 정수이다.

// 출력 : 수를 합이 최대가 나오게 묶었을 때 합을 출력한다. 정답은 항상 231보다 작다.

public class MaximumOfGroupingNumbers {
	public static void main(String[] args) {
		Scanner sc = new Scanner(System.in);
		int N = sc.nextInt();
		PriorityQueue<Integer> plusPq = new PriorityQueue<>();	// 양수 집합
		PriorityQueue<Integer> minusPq = new PriorityQueue<>();	// 음수 집합
		int one = 0;	// 숫자 1의 개수
		int zero = 0;	// 숫자 0의 개수
		for (int i = 0; i < N; i++) {
			int data = sc.nextInt();
